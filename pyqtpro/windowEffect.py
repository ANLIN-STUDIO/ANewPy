...
class WindowEffect:
    ...
    def setAcrylicEffect(self, hWnd: int, gradientColor: list | tuple = (0, 0, 0, 155),
        ...
        """
        # 亚克力混合色
        gradientColor = ANewPy.RGB2Hex(gradientColor[:-1])[1:] + str(gradientColor[-1])
...
def ClearEffectPreposition(self):
    self.setStyleSheet("")


@ANewPy.paras_checker(gradientColor=[ANewPy.Rule.list_structure, (int, int, int, int)])
def AcrylicEffectPreposition(self, gradientColor: list | tuple = (0, 0, 0, 155),
                             isEnableShadow: bool = True, animationId: int = 0):
    """ 开启亚克力效果
...
def moveWindow(self):
    ...
    win32api.SendMessage(getWinId(self), win32con.WM_SYSCOMMAND,
